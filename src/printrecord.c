...
unsigned int printrecord_cat(stralloc *out,const char *buf,unsigned int len,unsigned int pos,const char *q,const char qtype[2])
{
  ...
  if (dns_domain_todot_cat(out,d) <= 0) return 0;
  ...
  if (x) {
    ...
    stringlen = ip4_fmt(ip4str,misc);
    if (!stralloc_catb(out,ip4str,stringlen)) return 0;
  }
    ...
    while (datalen--) {
      ...
      if ((misc[0] >= 32) && (misc[0] <= 126) && (misc[0] != '\\')) { // allow spaces
