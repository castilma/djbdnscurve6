...
void die_truncated()
{
  logmsg(WHO,111,FATAL,"truncated request");
}
void die_netwrite()
{
...
void die_outside()
{
  logmsg(WHO,111,FATAL,"unable to locate information in data.cdb");
}
void die_cdbread()
{
  logmsg(WHO,111,FATAL,"unable to read data.cdb");
}
void die_cdbformat()
{
  logmsg(WHO,111,FATAL,B("unable to read data.cdb: ","format error"));
}
...
void axfrcheck(char *q)
{
  ...
  for (;;) {
    if (!axfr[i] || (axfr[i] == '/')) {
      if (i > j) {
        if (dns_domain_fromdot(&axfrok,axfr + j,i - j) <= 0) nomem();
  ...
  logmsg(WHO,111,FATAL,"disallowed zone transfer request");
}
...
int main()
{
  ...
  for (;;) {
    ...
    if (len > MSGSIZE) logmsg(WHO,111,FATAL,"excessively large request");
    netread(buf,len);

    pos = dns_packet_copy(buf,len,0,header,12); if (!pos) die_truncated();
    if (header[2] & 254) logmsg(WHO,111,FATAL,"bogus query");
    if (header[4] || (header[5] != 1)) logmsg(WHO,111,FATAL,"bogus query");
    ...
    if (byte_diff(qclass,2,DNS_C_IN) && byte_diff(qclass,2,DNS_C_ANY))
      logmsg(WHO,111,FATAL,"bogus query: bad class");
