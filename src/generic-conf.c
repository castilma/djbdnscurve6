...
void init(const char *d,const char *f)
{
  ...
  umask(022);
  if (mkdir(dir,0700) == -1)
    logmsg(fatal,111,FATAL,B("unable to create: ",dir));
  if (chmod(dir,03755) == -1)
    logmsg(fatal,111,FATAL,B("unable to set mode of: ",dir));
  if (chdir(dir) == -1)
    logmsg(fatal,111,FATAL,B("unable to switch to: ",dir));
}

void fail(void)
{
  logmsg(fatal,111,FATAL,B("unable to create: ",dir,"/",fn));
}
