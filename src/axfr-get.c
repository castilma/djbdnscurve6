...
void die_usage(void)
{
  logmsg(WHO,100,USAGE,"axfr-get zone fn fn.tmp");
}
void die_generate(void)
{
  logmsg(WHO,111,FATAL,"unable to generate AXFR query");
}
void die_parse(void)
{
  logmsg(WHO,111,FATAL,"unable to parse AXFR results");
}
...
{
  logmsg(WHO,111,FATAL,"unable to read from network");
}
void die_netwrite(void)
{
  logmsg(WHO,111,FATAL,"unable to write to network");
}
void die_read(void)
{
...
void die_write(void)
{
  logmsg(WHO,111,FATAL,B("unable to write: ",fntmp));
}
...
unsigned int doit(char *buf,unsigned int len,unsigned int pos)
{
  ...
  if (len - pos < dlen) { errno = EPROTO; return 0; }
  len = pos + dlen;
  ...
  if (byte_equal(data,2,DNS_T_SOA)) {
    ...
    if (dns_domain_todot_cat(&line,d1) <= 0) return 0;
    if (!stralloc_cats(&line,"|")) return 0;
    if (dns_domain_todot_cat(&line,d2) <= 0) return 0;
    if (!stralloc_cats(&line,".|")) return 0;
    if (dns_domain_todot_cat(&line,d3) <= 0) return 0;
    if (!stralloc_cats(&line,".")) return 0;
    for (i = 0; i < 5; ++i) {
    ...
    if (!stralloc_copys(&line,"&")) return 0;
    if (byte_equal(d1,2,"\1*")) { errno = EPROTO; return 0; }
		if (dns_domain_todot_cat(&line,d1) <= 0) return 0;
    if (!stralloc_cats(&line,"||")) return 0;
    ...
    if (!stralloc_catb(&line,ipstr,ip6_fmt(ipstr,data))) return 0;
  }
  else {
    unsigned char ch;
    unsigned char ch2;
...
int main(int argc,char **argv)
{
  ...
  if (!*++argv) die_usage();
  if (dns_domain_fromdot(&zone,*argv,str_len(*argv)) <= 0) die_generate();
  ...
  if (!numanswers) { errno = EPROTO; die_parse(); }
  pos = x_getname(packet.s,packet.len,pos,&d1);
  if (!dns_domain_equal(zone,d1)) { errno = EPROTO; die_parse(); }
  pos = x_copy(packet.s,packet.len,pos,out,10);
  if (byte_diff(out,4,DNS_T_SOA DNS_C_IN)) { errno = EPROTO; die_parse(); }
  pos = x_skipname(packet.s,packet.len,pos);
  pos = x_skipname(packet.s,packet.len,pos);
  ...
  if (rename(fntmp,fn) == -1)
    logmsg(WHO,111,FATAL,B("unable to move ",fntmp," to: ",fn));
  _exit(0);
}
